...
void IndexBuffer::Create(const UINT size, const UINT stride) {
	// strideの値によって、1つのイデックスのフォーマットを決める
	...
	// インデックスリソースの生成　=======================================
	// インデックスリソース用のヒープの設定
	D3D12_HEAP_PROPERTIES uploadHeapProperies{};
	uploadHeapProperies.Type = D3D12_HEAP_TYPE_UPLOAD;
	// インデックスリソースの設定
	...
	HRESULT hr = dxCommon->GetDevice()->CreateCommittedResource(
		&uploadHeapProperies, D3D12_HEAP_FLAG_NONE, &indexResourceDesc, 
